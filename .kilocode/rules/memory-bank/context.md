...
## 2026-07-24 Session — Data protection & GDPR compliance

- **Privacy policy page** (`/privacy`): static page covering data collection (auth email, profile, submissions, board posts, Reddit public data), localStorage-only preferences table, advertising/cookies (AdSense not currently active), third-party processors (Supabase, Vercel, Railway, Google OAuth), GDPR rights (access/rectification via account page; erasure deletes identity data while submissions are kept but anonymized via `ON DELETE SET NULL`), retention, contact.
- **Terms of service page** (`/terms`): static page covering data-accuracy disclaimer (self-reported salaries, simplified tax, COL estimates, AI briefs), account rules, submission guidelines (genuine knowledge only, no fabrication), bounties/reputation (no monetary value), jobs-board rules, acceptable use, liability limitation, changes clause.
- **Footer legal links**: `SiteFooter` now has inline Privacy + Terms links.
- **Account deletion** (`DELETE /api/account/delete` + two-step danger-zone button on `/account`): verifies caller via `auth.getUser(token)`, then `auth.admin.deleteUser(userId)`. Schema cascades handle the rest: `profiles` (cascade from auth.users), `school_members`/`board_posts`/`board_post_flags` (cascade from profiles), and `salary_records`/`col_items` (ON DELETE SET NULL → kept but anonymized, matching the user's requirement that submitted data survives without identity linkage).
- **Cookie/ad consent banner** (`ConsentBanner.tsx`, mounted in `layout.tsx`): one-time localStorage banner (`si.cookie-consent` = accepted/declined) covering current localStorage-only usage and future ad gating; dispatches `si-consent-changed` event for ad components to listen on.
- **GoogleAdSlot disabled**: live `<ins>` rendering and `adsbygoogle.push()` hashed out; component always renders the placeholder. A commented-out re-enable block documents the consent gate (`getConsent() === "accepted"`) required before going live per ePrivacy/GDPR and AdSense policy.
- **Validation**: typecheck exit 0, lint exit 0, tests 84 pass / 1 pre-existing fail. Smoke-tested `/privacy`, `/terms`, `/account` (delete button renders) via dev server.
